...
- `GET /api/posts` → elanlar/materiallar (daxil olmuş; kursant yalnız hamıya + öz taqımına olanları görür). `POST /api/post-save` `{type: elan|tapsiriq|muhazire, title, text, teams?: [taqım...] ([] və ya hamısı = bütün taqımlar), deadline?}`, `POST /api/post-delete` `{id}`, `POST /api/post-update` `{id, type, title, text, teams?, deadline?}` (düzəliş; fayllar post-file-url/confirm/delete ilə), `POST /api/post-file-url` `{id, fname, size}` → `{url, fid}`, `post-file-confirm` `{id, fid, fname}`, `post-file-delete` `{id, fid}` (müəllim; istənilən format, məzmun yoxlanmır, ≤20 fayl, texniki hədd 200 MB, B2 açarı `posts/<id>-<fid>.<ext>`), `POST /api/post-file-link` `{id, fid, mode: view|download}` (daxil olmuş). Modul: `api/_posts.py`, baza: `db["posts"]` (ən çoxu 60).
