...
- `GET /api/posts` → elanlar/materiallar (daxil olmuş; kursant yalnız hamıya + öz taqımına olanları görür). `POST /api/post-save` `{type: elan|tapsiriq|muhazire, title, text, teams?: [taqım...] ([] və ya hamısı = bütün taqımlar), deadline?}`, `POST /api/post-delete` `{id}`, `POST /api/post-file-url` / `post-file-confirm` `{id, kind: docx|pptx|pdf, fname, size}` (müəllim; B2 açarı `posts/<id><ext>`), `POST /api/post-file-link` `{id, mode: view|download}` (daxil olmuş). Modul: `api/_posts.py`, baza: `db["posts"]` (ən çoxu 60).
